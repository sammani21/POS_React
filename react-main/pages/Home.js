    ...
    return (
        <div className="home-container">
            ...
            <div className="home-date-picker-container">
                <label className="home-date-picker-label">Select Date:</label>
                <DatePicker />
            </div>

            <button className="home-btn btn-primary" onClick={getProducts}>Refresh Products</button>

            {loading ? (
                <p className="loadingh">Loading...</p>
            ) : error ? (
                ...
                <ul className="home-product-list">
                    {products.map((product) => (
                        <li key={product.id} className="home-product-item">
                            <Link to={`/product/${product.id}`} className="home-product-link">{product.name}</Link>
                        </li>
                    ))}
                </ul>
            ) : (
                <p className="home-no-products-message">No products available</p>
            )}

            <form onSubmit={createProduct} className="home-product-form">
                ...
                <button type="submit" className="home-btn btn-submit">Save Product</button>
            </form>
        </div>
    );
};
